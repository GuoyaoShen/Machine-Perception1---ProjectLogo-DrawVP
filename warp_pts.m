...
for i=1:size(sample_pts,1)
    %calculate the new warped raw
    warped_new = H * [sample_pts(i,:)';1];
    %calculate lambda in H
    H_lambda = H(3,1) * sample_pts(i,1) + H(3,2) * sample_pts(i,2) + H(3,3);
    %normalize the new warped raw
    warped_new = warped_new(1:2,:)' / H_lambda;
    warped_pts = [warped_pts;warped_new];
end
end
